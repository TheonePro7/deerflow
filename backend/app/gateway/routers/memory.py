...
@router.get(
    ...
    description="List all LangMem memories for the current user.",
)
async def list_langmem() -> LangMemListResponse:
    """List all LangMem memories for the current user."""
    user_id = get_effective_user_id()
...
async def _fetch_langmem_memories(user_id: str) -> list[LangMemItem]:
    """Fetch all LangMem memories for a user."""
    try:
