...
| 文件 | 修改类型 |
|------|---------|
| `backend/packages/harness/deerflow/agents/memory/langmem_adapter.py` | **新增** |
| `backend/packages/harness/deerflow/agents/memory/updater.py` | 修改 |
| `backend/packages/harness/deerflow/agents/lead_agent/prompt.py` | 修改 |
...
```
对话结束 → 现有记忆系统（memory.json）→ 并行 → LangMem（langmem.db）
                                                      ↓
                                              SQLite + 语义分类
                                                      ↓
对话开始 → 现有记忆注入 + LangMem 记忆注入 → AI 系统提示词
```

### 16.2 记忆分类

LangMem 自动从对话中提取以下类型信息：
- 用户偏好（语言、风格、习惯）
- 工作领域和背景
- 技术决策和理由
- 项目需求和目标
...
| 测试项 | 结果 |
|--------|:----:|
| DeepSeek V4 Flash 兼容 | ✅ 正常 |
| 中文记忆提取 | ✅ 正常 |
| 记忆注入系统提示词 | ✅ 正常 |
| AI 根据记忆调整行为 | ✅ 4/5 通过（称呼主公、引用技术栈） |
| 不影响现有系统 | ✅ 零侵入代码 |

### 16.4 分支管理

```
main                         → v6.2-stable（稳定版）
feat/langmem-integration     → LangMem 实验分支
```

如需回退：`git checkout main && git reset --hard v6.2-stable`
...
| 2026-05-16 | **v7.0-rc** | **LangMem 记忆系统（实验性）** |
| 2026-05-16 | v7.0-rc.1 | 记忆管理器 UI（左侧导航增「记忆」页面，CRUD）|
| 2026-05-16 | v7.0-rc.2 | 修复消息换行 break-words + LangMem CRUD 403 |
| 2026-05-16 | v7.0-rc.3 | 修复 asearch 替代 alist + 记忆页面 UI 重做 |
