...
export function MentionPopover({
  ...
  useEffect(() => {
    if (!threadId) return;
    fetchThreadFiles(threadId).then((r) => setFiles(r.files));
  }, [threadId]);
  ...
  useEffect(() => {
    ...
    if (detected) {
      ...
      setOpen(true);
      setPopoverKey((k) => k + 1);
      // Directly enter skill selection when user types /skill
      if (detected.type === "/" && detected.query === "skill") {
  ...
  const matchingCommands = useMemo(() => {
    if (!state || state.type !== "/") return [];
    const q = state.query.toLowerCase();
    // If showing skill sub-selection, show ALL skills (cmdk's CommandInput handles filtering)
  ...
  const handleSelect = useCallback(
    (itemId: string) => {
      ...
      if (itemId.startsWith("skill:")) {
        ...
        // Replace /skill in input with the skill name
        const endPos = state.triggerPos + 6; // "/skill"
        onInsert(skillName, { start: state.triggerPos, end: endPos });
        setOpen(false);
        return;
      }
    },
  ...
  if (state.type === "/" && selectedSub === "skill") {
    return (
      <div className="mb-2 w-full max-w-full">
        <Command key={`cmd-${popoverKey}-skills`} className="border-border w-full rounded-lg border shadow-lg">
          <CommandInput
            ref={inputRef}
            placeholder="搜索技能..."
            className="h-9"
            ...
            <CommandGroup heading="技能列表">
              {matchingCommands.map((item) => (
                ...
                >
                  ...
                  <div className="grid min-w-0 grid-cols-[1fr] gap-y-0.5 items-start">
  ...
  return (
    <div className="mb-2 w-full max-w-full">
      <Command key={`cmd-${popoverKey}`} className="border-border w-full rounded-lg border shadow-lg">
        <CommandInput
          ref={inputRef}
          placeholder={
